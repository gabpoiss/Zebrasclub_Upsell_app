...
export class BaseItem extends React.Component {
    ...
    render() {
        ...
        const { url, icon, label, badge, subNavigationItems = [], secondaryAction, disabled, onClick, accessibilityLabel, iconBody, new: isNew, polaris: { intl }, selected: selectedOverride, } = this.props;
        ...
        const badgeMarkup = badge || isNew ? (<div className={styles.Badge}>
          <Badge status="new" size="small">
            {badge || intl.translate('Polaris.Badge.STATUS_LABELS.new')}
          </Badge>
        </div>) : null;
        const iconMarkup = iconBody ? (<span className={styles.Icon} dangerouslySetInnerHTML={{ __html: iconBody }}/>) : (icon && (<div className={styles.Icon}>
          <Icon source={icon}/>
        </div>));
...
tslib_1.__decorate([
    autobind
], BaseItem.prototype, "handleResize", null);
tslib_1.__decorate([
    autobind,
    memoize()
], BaseItem.prototype, "getClickHandler", null);
