...
import { Weekdays, Months, isDateBefore, isDateAfter, isSameDay, getWeeksForMonth, dateIsInRange, dateIsSelected, getNewRange, abbreviationForWeekday, } from '@shopify/javascript-utilities/dates';
...
export default function Month({ focusedDate, selected, hoverDate, disableDatesBefore, disableDatesAfter, allowRange, onChange = noop, onHover = noop, onFocus = noop, month, year, weekStartsOn, }) {
    ...
    const weekdays = getWeekdaysOrdered(weekStartsOn).map((weekday) => (<Weekday key={weekday} title={abbreviationForWeekday(weekday)} current={current && new Date().getDay() === weekday} label={weekday}/>));
